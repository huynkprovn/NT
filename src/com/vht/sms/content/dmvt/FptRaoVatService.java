...
public class FptRaoVatService extends ContentAbstract {
	static Log logger = LogFactory.getLog(FptRaoVatService.class);
	...
	protected Collection getMessages(DeliverMessage dm) throws Exception {
		...
		try {
			...
			logger.error(">>>Message timeout!!! RAOVAT " + e.getMessage());
			messageType = MyConst.CHARGE;
			contentType = MyConst.TEXT;
	...
	public static String callApiMsg(DeliverMessage dm, String code) {
		...
		try { 
			...
			requestList.add(new HttpHeader("phone", "0"+dm.getUserId().substring(2), false));
	...
	public static void main(String[] args) {
		FptRaoVatService dmvt = new FptRaoVatService();
		DeliverMessage dm = new DeliverMessage();
		dm.setId(123456);
		dm.setCommandCode("DTU");
		dm.setMessage("DTU 15301");
		dm.setMobileOperator("Viettel");
		dm.setServiceId("6789");
		dm.setUserId("84973686767");
		try {
			dmvt.getMessages(dm);
		} catch (Exception e) {
			 e.printStackTrace();
